...
{
	push_back(Sum());//додаЇмо суму в к≥нець масиву
	push_back(arithmeticMeanAbs());//додаЇмо середнЇ арифметичне в к≥нець масиву
}
double Array::firstNegativeNumber() const
{
	double firstNegativeNumber;

	for (int i = 0; i < size(); i++)
		if (elems[i] < 0)
		{
			firstNegativeNumber = elems[i];
			break;
		}
...
void Array::myTask()
{
	double firstNegativeNumber = this->firstNegativeNumber();

	for (int i = 0; i < size(); i++)
	{
		elems[i] /= ((firstNegativeNumber + elems[49]) / 2.);
	}
}